...
class Odometry:
    """处理nav_msgs/Odometry消息，用于获取机器人位置和速度信息"""
    
    def __init__(self, publisher: Publisher, topic: str = "/odom"):
    ...
    def subscribe(self, timeout: float = 5.0) -> Optional[dict]:
        ...
        Returns:
            包含位置和速度信息的字典，如果失败返回None
        """
        try:
            msg = self.publisher.subscribe_once(self.topic, timeout)
            if msg:
                return self._parse_odometry_message(msg)
            return None
    ...
    def get_velocity(self, timeout: float = 5.0) -> Optional[dict]:
        """
        获取当前速度信息
        ...
        try:
            msg = self.publisher.subscribe_once(self.topic, timeout)
            if msg:
                return self._extract_velocity(msg)
            return None
        except Exception as e:
            print(f"获取速度信息失败: {e}")
            return None

    def _parse_odometry_message(self, msg: dict) -> dict:
        """解析完整的里程计消息"""
        try:
            # 提取位置信息
            ...
            return {
                ...
                'child_frame_id': msg.get('msg', {}).get('child_frame_id', 'unknown')
            }
        except Exception as e:
            print(f"解析里程计消息失败: {e}")
            return {}
    ...
    def _extract_position(self, msg: dict) -> Optional[dict]:
        ...
        try:
            ...
            # 获取frame_id
            frame_id = msg.get('msg', {}).get('header', {}).get('frame_id', 'odom')
    ...
    def _extract_velocity(self, msg: dict) -> Optional[dict]:
        """从消息中提取速度信息"""
        try:
            twist = msg.get('msg', {}).get('twist', {}).get('twist', {})
